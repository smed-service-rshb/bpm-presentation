...
import {CLIENT_FIO_CONTEXT_KEY, CLIENT_CHECK_RESULT_CONTEXT_KEY} from './constants'

class ConfirmClientForm extends React.Component {
    completeTask = (action) => () => {
    ...
    render = () => {
        ...
        return (
            <Panel label="Форма подтвержления создания клиента." dataId="confirm-client-panel">
                <Form buttons={fromButtons} dataId="confirm-client-form">
                    <Field title="ФИО клиента">{this.props.bpmContext[CLIENT_FIO_CONTEXT_KEY]}</Field>
                    <Field title="Результат проверки клиента">{this.props.bpmContext[CLIENT_CHECK_RESULT_CONTEXT_KEY]}</Field>
                    Информация о задаче:
